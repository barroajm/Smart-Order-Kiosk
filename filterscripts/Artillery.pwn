...
public OnPlayerCommandText(playerid, cmdtext[])
{
		...
		if(strcmp(cmd, "/addartillery", true) == 0)
  {
			if(IsPlayerConnected(playerid))
   {
			    ...
			    new tmp[64];
			    format(tmp, 64, "New Altillery Added! ID: %d", AddBase(px, py, pz - 0.5));
		...
		if(strcmp(cmd, "/removeartillery", true) == 0)
  {
      	if(IsPlayerConnected(playerid))
			{
				...
				if(!strlen(tmp)) {
				    SendClientMessage(playerid, COLOR_RED, "Use /Removebase <id>");
					return 1;
				}
				RemoveBase(strval(tmp));
				SendClientMessage(playerid, COLOR_BLUE, "Artillery Destroyed!");
      }
			else
		...
		if(strcmp(cmd, "/sscan", true) == 0)
		{
			...
			if(HasPlayerControl(playerid, base, 1))
			{
				...
				SendClientMessage(playerid, COLOR_RED, "You do not have the permittance to do that!");
		    return 1;
		}

		if(strcmp(cmd, "/setrotate", true) == 0)
		{
			new tmp[32];
			tmp = strtok(cmdtext, idx);
		...
		if(strcmp(cmd, "/setmissilecount", true) == 0)
		{
            new base = strval(strtok(cmdtext, idx));
            if(HasPlayerControl(playerid, base, 0))
			{
				firecount[base] = strval(strtok(cmdtext, idx));
			    SendClientMessage(playerid, COLOR_YELLOW, "Missile Count set!");
		...
		if(strcmp(cmd, "/setangle", true) == 0)
		{
            ...
            if(HasPlayerControl(playerid, base, 0))
			{
				...
				new tmp[64];
				format(tmp, 64, "Artillery %d: Angle adjusted to %f! Distance changed to ~%dm", base, aangle[base], floatround((floatpower(floatround(apower[base]), 2) / 10) * floatsin(floatround(2 * aangle[base]), degrees)));
		...
		if(strcmp(cmd, "/setpower", true) == 0)
		{
            ...
            if(HasPlayerControl(playerid, base, 0))
			{
				...
				new tmp[64];
				format(tmp, 64, "Artillery %d: Missile Explotion Power set to %f! Distance changed to ~%dm", base, apower[base], floatround((floatpower(floatround(apower[base]), 2) / 10) * floatsin(floatround(2 * aangle[base]), degrees)));
				SendClientMessage(playerid, COLOR_YELLOW, tmp);
				t = 0;
            }
			else
		...
		if(strcmp(cmd, "/setdelay", true) == 0)
		{
            ...
            if(HasPlayerControl(playerid, base, 0))
			{
				...
				new tmp[64];
				format(tmp, 64, "Artilerry %d: Missile Delay adjusted to %d!", base, delay[base]);
				SendClientMessage(playerid, COLOR_YELLOW, tmp);
			}
			else
				SendClientMessage(playerid, COLOR_RED, "You do not have the permittance to do that!");
		...
		if(strcmp(cmd, "/launchmissiles", true) == 0)
		{
			new tmp[32] = "0";
			new base;
			while(strlen(tmp))
			{
				...
				if(strlen(tmp) > 0) {
                    ...
                    if(HasPlayerControl(playerid, base, 0)) FireStationaryArtillery(playerid, base);
					 	...
					 	SendClientMessageToAll(COLOR_GREEN, "MILITARY: Military Artillery has been launch and Chasing it's Target!");
			}
			return 1;
		}

		if(strcmp(cmd, "/checkmissile", true) == 0)
		{
			new base = strval(strtok(cmdtext, idx));
            if(HasPlayerControl(playerid, base, 0))
			{
		...
		if(strcmp(cmd, "/targetlock", true) == 0)
		{
			new base = strval(strtok(cmdtext, idx));
            if(HasPlayerControl(playerid, base, 0))
			{
			   	if(followtimer[base] == -1) {
					...
					format(tmp, 64, "Artillery %d: Locked the Target %d", base, followtarget[base]);
					SendClientMessage(playerid, COLOR_BLUE, tmp);
					return 1;
				}
			}
		...
		if(strcmp(cmd, "/unlocktarget", true) == 0)
		{
			new base = strval(strtok(cmdtext, idx));
            if(HasPlayerControl(playerid, base, 0))
			{
			    ...
			    followtimer[base] = -1;
			    format(tmp, 64, "Artillery %d: Missile Unlock following Target %d", base, followtarget[base]);
				SendClientMessage(playerid, COLOR_BLUE, tmp);
            }
			else
				SendClientMessage(playerid, COLOR_RED, "You do not have the permittance to do that!");
		...
		{    //still some debugging stuff - can be used for aiming ;)
			new Float:tax;
			new Float:tay;
			new Float:taz;
			new tmp[64];
	...
	if(abase[base] > 0)
	{
		...
		while((id < MAX_BOMBS) && (bombcount[base] < firecount[base]))
		{
			if(bomb[base][id] == 0)
			{
				...
				bombcount[base] ++;
				SetTimerEx("FireBomb", delay[base] * bombcount[base] + 150, 0, "iii", playerid, base, id);
			}
			id ++;
		}
	}
...
public MoveBomb(base, id)
{
	...
	if(lastcurz[base][id] != 0.0) SetObjectRot(bomb[base][id], atan2(curz[base][id] - lastcurz[base][id], xo[base][id] - lastcurx[base][id]), 0, -1 * rot[base][id]);
		 ...
		 SetObjectRot(bomb[base][id], alpha[base][id], 0, -1 * rot[base][id]);
	...
	for(new i = 0; i < MAX_PLAYERS; i++) {
		...
		{
	        new tmp[64];
			format(tmp, 64, "Artillery %d: Missile %d hit ground", base, id);
...
public FireBomb(playerid, base, id)
{
		...
		new tmp[64];
		format(tmp, 64, "Artillery %d: Missile Launch %d fired!", base, id);
		SendClientMessage(playerid, COLOR_YELLOW, tmp);
...
public MoveScan(base, playerid)
{
	...
	else if(key3 == 128)//press a
	{
		scanx[playerid] = scanx[playerid] - 0.02 * scanz[playerid];
	}
	if(key2 == 8 & 65536)//press y
	{
		scany[playerid] = scany[playerid] - 0.02 * scanz[playerid];
	}
	else if(key2 == 128 && 131072)//press n
	{
		scany[playerid] = scany[playerid] + 0.02 * scanz[playerid];
	}
	if(key1 == 1024 && 128)//press numpad2
	{
		scanz[playerid] = scanz[playerid] + 1;
	}
	else if(key1 == 4 && -128)//press Nunpad+/2
	{
		scanz[playerid] = scanz[playerid] - 1;
	}
	SetPlayerCameraPos(playerid, scanx[playerid], scany[playerid] + 0.01, scanz[playerid]);
    ...
    if(key1 == 2 && scanaimed[playerid] != 1)
	{
		...
		else SendClientMessage(playerid, COLOR_YELLOW, "Wait a moment, the Artillery is still rotating...");
	}
